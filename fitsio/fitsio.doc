...
FITSIO is a machine-independent library of Fortran-77 subroutines for
...
commonly used computers and workstations. This version of FITSIO
...
{\bf legacy.gsfc.nasa.gov} in the {\bf software/fitsio/fortran} 
subdirectory.
...
The author would like to thank the many individuals who have used or
tested FITSIO and have offered valuable suggestions to improve the
FITSIO package.  These include:

Steve Allen, Keith Arnaud, Kent Blackburn, G Bodammer, Romke Bontekoe,
Robin Corbet, Lucio Chiappetti, Richard Fink, Emily Greene, Cheng Ho,
Phil Hodge, Jim Ingham, Mark Levine, Todd Karakaskian, Edward King,
Scott Koch, Don Jennings, Claire Larkin, Rob Managan, John Mattox,
Carsten Meyer, Stefan Mochnacki, Bruce O'Neel, Clive Page, Arvind
Parmar, Jeff Pedelty, Tim Pearson, Chris Rogers, Arnold Rots, Barry
Schlesinger, Robin Stebbins, Allyn Tennant, Peter Teuben, Doug Tody,
Steve Walton, Dan Whipple, and Nelson Zarate.

*II.  Creating the FITSIO Library

**A.  Machine Specific Versions

The FITSIO package consists of about 30000 lines of Fortran source code
contained in about 350 subroutines.  The large majority of subroutines
are written in strict Fortran-77 (except for the occasional use of
INTEGER*2 variables) and are contained in the file named fitsio.f.
These subroutines can be compiled and run on any machine that has a
Fortran-77 compiler.  A second file, called fitsfort.f, contains
additional general subroutines that are used in all cases except for
the IRAF/SPP version (which does not use Fortran read and write
statements).

To complete the FITSIO library, an additional set of machine-specific
subroutines must be added.  These subroutines allow for
compiler-specific differences in the file OPEN statement, the
subroutine to determine the current date, and a few other non-portable
aspects of Fortran-77.

Recently fitsio has been ported to be compilant with Fortran-90
compilers.  The fitsf90.f source file is intended to be used with any
Fortran-90 compiler, but it has only been tested on few F90
compilers.  

The build scripts included in the FITSIO release will automatically
include the correct machine-specific Fortran-77 source file, but for
reference, the following table shows which file should be used on
various machines:
...
Note that IBM mainframe computers and some older compilers for IBM PC
clones are no longer supported by this version of FITSIO.  The
previous 4.14 version of FITSIO may still be used on these platforms.
The fitsf90.f file should work with most newer Fortran-90
compilers on IBM PCs.

A version of FITSIO is also available which runs in the IRAF
environment and provides a set of SPP callable interface routines. See
the later section of this guide for more information on this IRAF
version.

**B.  Building the Fortran Library

A set of Make files and command files are provided with FITSIO to
automate the process of building the FITSIO library on most machines.
The following sections describe the build procedure for each type of machine.
...
library.  The build process splits each routine in the source files
into a separate disk file in a scratch subdirectory and may take some
time to finish, depending on the speed of the processor.

On HP-UX systems using an older Fortran-77 compiler, programs that call
FITSIO should be compiled and linked with the +U77 switch (to bring in
the idate library function) and the +E5 switch (to enable the same FITS
file to be attached to multiple Fortran unit numbers).  The idate
subroutine is only needed when writing the DATE FITS keyword with the
FTPDAT subroutine.  For example, to compile the testprog.f program
type:
-
...
The makevms.com and command file may be used to build the FITSIO
library on DEC Alpha/OSF and VAX/VMS machines (e.g., type `@makevmv' on
the command line).  Note that the vmsieeed.mar macro routine is used on
VAX/VMS systems to perform the translations between Vax floating point
formats and IEEE floating point formats.

FITSIO supports both the G\_float (default) and D\_float compiler options
on Alpha VMS systems.  The vmsieee.c routines perform the the IEEE
translations on Alpha VMS machines when the /float=D\_float compiler
option is invoked.  Th makevmsd.com file can be used to build FITSIO
with this option.

***3.  IBM Compatible PCs running DOS or Windows

This version of FITSIO currently requires a Fortran-90 compiler to
build on an IBM PC running DOS or Windows.  The previous 4.14 version
of FITSIO may still be used with the older Fortran-77 compilers.  In
fact it is probably not difficult to port this new version FITSIO to
run with most older compilers;  anyone wishing to do so should contact
the author for assistance.

To build the FITSIO library with a Fortran-90 compiler, one needs to
compile all the subroutines in fitsio.f, fitsfort.f, and fitsf90.f.
It usually results in smaller and more efficient code if each
subroutine in these source files is split into a separate file before
compiling, rather than compiling the entire large source file at once.
A program called split.f is included with the fitsio distribution which
will split the fitsio.f and fitsfort.f subroutines into separate
files.

The exact commands to compile and build the library depend on the
compiler, but the following example for the Microsoft PowerStation
Fortran compiler illustrates the sequence of steps that are required:
-
...
The object module library, FITSIO.LIB, may then be used when linking
any program that calls the FITSIO subroutines.

***4.  Using Macintosh PCs

There are currently 2 versions of FITSIO for use with different Fortran
compilers on Macintosh PCs.  The fitsmac.f file is for the Absoft
compiler, and the fitsmacls.f file should be used with the Language
Systems Fortran (V3.3) compiler.  The MacMakefiles.sit.hqx file
contains libfitsio.make which will build libfitsio.f.o.  Use Stuffit
Expander (freeware, see any good archive site) to unbinhex and unstuff
these files.  You can change FMakeOptions and FLinkOptions if you
want.  For instance, set
-
FMakeOptions = ... -sym
FLinkOptions = ... -sym ON
-
to use SADE. FITSIO has not been extensively tested on Mac systems, so
please report any problems or additional suggestions for successfully
building FITSIO on Macs to the author.

***5.  Using the f2c Compiler

The widely available f2c program may be used to convert the fortran
FITSIO source files into C code, which then may be processed by a C
compiler.  This avoids the need to have a Fortran compiler to build the
FITSIO library.  The fitsio.f file should process through f2c without
any problems, but the machine-dependent version of FITSIO may have a
few non-standard fortran statements which f2c will not understand.  One
such case is the use of the IDATE subroutine, or equivalent, to get the
system date which is used in the ftgsdt subroutine.  The easiest
workaround to this problem is to simply comment out this call and then
avoid calling the ftpdat subroutine, which is the only FITSIO
subroutine to use this function.

***6.  The IRAF SPP Compatible Version of FITSIO

An IRAF compatible version of FITSIO was developed in collaboration
with Doug Tody at NOAO.  This version of FITSIO uses the IRAF VOS
(Virtual Operating System) file I/O calls to access the FITS files
rather than using Fortran I/O,

The following source files are needed to build the IRAF compatible version
of FITSIO:
-
...
-
The SPP version of FITSIO consists of 3 layers of code.  At the bottom
there is the fitsspp.x SPP source code which contains the IRAF-specific
subroutines to read and write data to the FITS file.  The middle layer
of subroutines are contained in the fitsio.f file which is the same
file used in every version of FITSIO.  This layer is completely portable and
contains no machine or environment dependencies.  Finally, a thin interface
layer of SPP subroutines are provided in the fitssppb.x file.  This top layer
of routines is provided for the convenience of SPP programmers and simply
performs the conversion between SPP character variables and Fortran character
strings. 

A sample mkpkg file is provided which will build the FITSIO library in IRAF.
There are 2 versions of the fitsspp.x file; the fitssppvms.x file is used
on VAX and Alpha VMS machines, while the fitsspp.x file is used on
all other platforms.  Note that the `fitsio.f' file must be renamed
to `fitsio.for' for the mkpkg to function properly.

Once the FITSIO library has been built, IRAF programmers may then call
either the SPP or the Fortran subroutine interface to FITSIO; both
interfaces end up calling the same low-level SPP VOS interface routines
to actually read or write the FITS files.   IRAF programs written in
Fortran (using either the IRAF IMFORT or the STScI F77 interface to
access the IRAF VOS) can simply call the Fortran subroutine interface
to FITSIO as specified in later sections of this document.

Programs written in SPP, on the other hand, may call the SPP specific
layer of FITSIO subroutines.  These routines are identical to the
Fortran subroutines except that the SPP subroutines have names
beginning with 'fs' rather than 'ft' (e.g., 'call fsopen' rather than
'call ftopen').  The main purpose of this thin layer of SPP routines is
to convert any SPP character variables into Fortran character strings
before calling the corresponding FITSIO Fortran subroutine.  Similarly,
any  character strings that are returned by the Fortran FITSIO
subroutine are converted into SPP CHAR variables before being passed
back to the calling SPP routine.

SPP programmers, therefore, should follow the definition of all the
FITSIO interface subroutines given in this document, except that the
SPP routines have names beginning with 'fs' rather than 'ft', and SPP
CHAR variables should be substituted for any Fortran character string
variables in the subroutine calling arguments. SPP programmers also
need to be aware of the character string length definitions that are
used throughout the SPP FITSIO interface as defined in the fitsio.h
include file. The SPP version of FITSIO behaves identically to the
Fortran version and has exactly the same calling sequences, except for
the following special exceptions:

>*  FSPKNx:  This family of subroutines only supports a single comment string,
>   not an array of comment strings as in the Fortran routines.

>*  FSPCLS, FSGCVS, and FSGCFS:  These 3 routines all have an additional
    argument not present in the corresponding Fortran routines.
    This integer argument, DIM1, follows the character array
    argument and specifies the size of the 1st dimension of
>   the 2-dimensional CHAR array, e.g.,
-
    call fspcls(ounit, frow, felem, nelem, nulval, array, DIM1, anynul, status)
-

**C.  Testing the FITSIO Library

The FITSIO library should be tested by building and running
the testprog.f program that is included with the release. 
...
-
    ...
    $ run testprog
-
The testprog program should produce a long set of diagnostic output
messages ending with an 'OK' status, identical to those contained in
the testprog.out file.  The output FITS file, called testprog.fft,
should be identical to the testprog.std FITS file included in this
release.  The 'cmp' command shown above should not report any
differences between these 2 files except that on a few systems (e.g.,
HP-UX and LINUX) a few bytes may differ where a floating point number
is formated with a leading blank instead of a leading zero in front of
the decimal place (e.g. ' .000000' instead of '0.000000').
...
-
**E.  Calling FITSIO from C programs  (Obsolete Section)

[Note added March 1996:  This section is now obsolete.  C programmers
should use the CFITSIO library, written entirely in ANSI-C
rather than the CFORTRAN macros described below.]

C programmers may call the Fortran FITSIO subroutines by using a set of
macro definitions included in the cfitsio.h header file that is
distributed as part of the FITSIO package. This header file defines a C
macro corresponding to every Fortran subroutine in the FITSIO user
interface. These macros provide a machine-independent interface between
C and Fortran by using the CFORTRAN package (actually a C header file)
developed by Burkhard Burow (burow@vxdesy.cern.ch, U. of Toronto).
...

*IV.  Basic FITSIO Conventions

**A.  Current Header Data Unit (CHDU)

A basic concept used throughout FITSIO is that of the current Header
Data Unit (CHDU) within the FITS file. The FITSIO subroutines which read
...
(i.e., the primary array).  FITSIO subroutines are provided to move
...
**C.  Subroutine Families and Datatypes
...
**G.  Error Status Values and the Error Message Stack
...
alone. It it recommended that the error message stack be printed out
...
**H.  Variable-Length Array Facility in Binary Tables
...
in the FITS header.  When creating a new binary table, the initial value of
PCOUNT should usually be set to zero. FITSIO will recompute the size of
...
(with ftibin). In this specific case the correct final value of PCOUNT
(or a larger value) must be specified when the HDU is initially created 
so that the correct amount of space will be inserted into the FITS file.
...
flag the desired elements as undefined. (Do not used the FTPCNx family
...
(e.g., TFORM = '1PA') only a single character string is read or
...
modification.  This does not work for double precision values
on VAX/VMS machines, however,
...
value in the FITS file, so programmers in general to not have to be
...
**E.  FITSIO Array Size Limitations

In general, FITSIO places no limits on the sizes of the FITS files that
...
set to 20.   To change this limit, one must globally edit the following
parameter statement:
-
        parameter (nb = 20)
-
throughout the fitsio.f and the machine-specific source files.  The
number of physical buffers, as defined by the 'pb' parameter in the
machine specific source file, must be set greater than or equal to the
value of 'nb'.   Increasing the value of 'pb' will also require that
the size of the ftbuff common block be modified accordingly.

2.  The total number of FITS table columns that can be accessed by
FITSIO, when summed over the current HDU in all the open FITS files,
cannot exceed the value of the 'nf' parameter in the fitsio.f source
code.  This is currently set to 3000, which, for example, will allow
FITSIO to simultaneously read or write 3 FITS tables with 999 columns
each, or 20 tables with 150 columns each, or any other combination that
does not exceed 20 files and 3000 total columns.  (A FITS primary array
or image extension is equivalent to a 2 column table).  FITSIO
allocates 56 bytes of memory for each column to store various
descriptive information so substantially raising or lowering the value
of 'nf' can have a significant effect on the total amount of computer
memory required to run a program.

3.  The maximum number of extensions that can be read or written
in a single FITS file is current set to 512 as defined
by the 'ne' parameter statements throughout the fitsio.f and
machine specific source code files.  This value may be increased
if necessary, but the access times to the later extensions in such
files may become very long.

4.  FITSIO can handle FITS files up to about 2.1 GB in size which is
