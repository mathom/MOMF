...
# Note that in this particular implementation, the gsize parameter
# is guaranteed to have a value of 1, which simplifies the logic
...
begin
	# Write the data. Probably won't work on a VAX
        call ftpbyt (iunit, gsize, array, status)
        do  i=2,ngroup  {
                        call ftmoff(iunit,offset,true,status)                
%                       call ftpbyt(iunit,gsize,array(i),status)
        }
end
...
begin
	# Get the data. Just call ftgbyt (probably won't work on VAX)
        call ftgbyt (iunit, gsize, array, status)
        do  i=2,ngroup  {
                        call ftmoff(iunit,offset,false,status)                
%                       call ftgbyt(iunit,gsize,array(i),status)
        }
end
