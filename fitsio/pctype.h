...
#define PCINT_cfSTR(N,T,A,B,C,D)
...
#define PCLOGICAL_cfSTR(N,T,A,B,C,D)  _(CFARGS,N)(T,PCLOGICAL,A,B,C,D) 
...
#undef   STRING_cfV
#undef   PSTRINGV_cfV
#define  STRING_cfV(T,A,B) struct {unsigned int clen, flen;} B;
#define  PSTRINGV_cfV(T,A,B) struct {char *fs; unsigned int sizeofA, flen;} B;
#endif
...
#ifdef vmsFortran
#define    PZTRING_cfV(T,A,B) static fstring B={0,DSC$K_DTYPE_T,DSC$K_CLASS_S,NULL};
...
#else
#define  PZTRING_cfV(T,A,B)  int     B;
#define APATRING_cfA(M,I,A,B,silA)   \
...
#define  PZTRING_cfSTR(N,T,A,B,C,D) _(CFARGS,N)(T,PZTRING,A,B,C,D)
