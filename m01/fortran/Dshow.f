...
c     
c---------------------------------------------
c      First-version: DAOshow.f
c---------------------------------------------
c      Hans Kjeldsen - MOMF version 2.1
c      Aarhus University 1992 - Denmark
c---------------------------------------------
c
c  This routine is used to select stars suitable for
c  MOMF.
c
       ...
       write(6,*)'Saturation-limit (after biassubtraction) in ADUs:'
       write(6,*)'The used limit will be 15 % lower'
       read(5,*)xsatur
       nsatur=nint(0.85*xsatur)
...
c   SGI convention defines record length given in words = 4 bytes
     ...
     c          recl=Nrecl,status='old')
          ...
          open (10,file='Dstars2',status='old')
          open (11,file='Dstars',status='new')
...
c
c Generation of screen-output in order to get a first
c impression on the local field
c
...
888           format(a12,20a2)
       ...
       write(6,*)
     .  'Can this star be used as a coo.star ? (2=stop/1=y/0=n):'
       read(5,*)nnn
