...
c
	parameter (NDIM=4096)
	integer*2 image(1:NDIM,1:NDIM),big(1:NDIM,1:NDIM)
        integer idata,scale
	character*80 name
	common /subset/ mxbig, mybig, icx, icy
        common big
c
...
c   scale < 0 means FITS file
     ...
     c          recl=nrecl,status='old')
          ...
          read(10,rec=1) ((big(i,j),i=1,mxbig),j=1,mybig)
          close(10)
	else 
          print 334
...
c
c   choose subset of image
c
	  do i=1,mmx
	  do j=1,mmy
	    image(i,j) = big(icx+i-1,icy+j-1)
	  enddo
	  enddo
c
        print 333
